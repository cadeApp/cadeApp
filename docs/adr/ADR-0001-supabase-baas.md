...
2. **Revelación progresiva de datos personales (`D3` y `D15`):** Los datos sensibles y coordenadas exactas del retiro y la entrega (`recipient_name`, `recipient_phone`, `dropoff_address`, `pickup_address`, `pickup_lat`, `pickup_lng`, `dropoff_lat`, `dropoff_lng`, `recipient_consent_declared`) se aíslan físicamente en `public.delivery_request_contacts` y jamás deben viajar al navegador antes del emparejamiento (`matched`), siendo visibles únicamente para el comercio dueño (`merchant_id`), el repartidor de la oferta aceptada (`accepted_offer_id`) y el administrador (`app_private.is_admin()`).
...
- **Separación de datos personales (`delivery_request_contacts`):**
  - La tabla `public.delivery_requests` (`supabase/migrations/20260922031435_schema_v1.sql:83`) contiene los campos operativos públicos de la bolsa (`merchant_id`, `status`, `pickup_zone_id`, `dropoff_zone_id`, `package_type`, `notes`, `recipient_payment_method`, `needs_change`, `cash_change_amount`, `approx_distance_m`, `route_distance_m`, `expires_at`, `accepted_offer_id`) para que los repartidores aprobados puedan cotizar conociendo las zonas (`public.zones`) y las distancias sin acceder a la identidad ni al domicilio exacto del destinatario (**`D3`** y **`D15`**).
  - **Estado actual de `delivery_requests.notes` (Decisión abierta `H02` y residual `PR56-H13`):** En el esquema mergeado (`supabase/migrations/20260922031435_schema_v1.sql:90`), la columna `notes` reside en `public.delivery_requests`. Dado que la política `delivery_requests_select_courier` (`supabase/migrations/20260922051650_rls_v1.sql:270`) habilita `SELECT` a nivel fila completa para repartidores aprobados (`app_private.is_approved_courier()`) cuando `status = 'published'` y no existen `GRANT SELECT (...)` por columna (`PR56-H13`, agendado para `T-106`), el texto de `notes` es legible en la bolsa antes de `matched`. Mientras `Lautaro073` resuelve la decisión abierta **`H02`** (evaluar si `notes` se migra a `delivery_request_contacts`, si se acota su proyección en `T-106`, o si permanece como nota operativa pública), queda prohibido por regla de negocio cargar direcciones exactas o teléfonos en `notes`.
  - La tabla `public.delivery_request_contacts` (`supabase/migrations/20260922031435_schema_v1.sql:111`) almacena `request_id`, `pickup_address`, `pickup_lat`, `pickup_lng`, `dropoff_address`, `dropoff_lat`, `dropoff_lng`, `recipient_name`, `recipient_phone` y `recipient_consent_declared`.
...
- **Riesgo 3 — Exposición de notas libres en `delivery_requests.notes` antes de `matched` (`H01` / `H02`):**
  - *Mitigación:* Regla de validación y advertencia explícita en UI de comercio para no incluir teléfonos ni direcciones exactas en `notes`, quedando abierta la decisión `H02` de `Lautaro073` y la tarea `T-106` (`PR56-H13`) para acotar columnas en el feed.
